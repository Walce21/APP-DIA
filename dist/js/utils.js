...
function linearRegression(data) {
    ...
    // --- INÍCIO DA CORREÇÃO ---
    // Se não há variação nos dados, o denominador é zero e a correlação é nula (R²=0), não 1.
    if (r2_denominator === 0) {
...
// --- INÍCIO: FUNÇÃO DE REGRESSÃO POLINOMIAL (P260625B-1.1) ---
/**
 * Calcula uma regressão polinomial de 2ª ordem (quadrática) para um conjunto de dados.
 * Adapta os dados para a forma y = a*x^2 + b*x + c.
 * @param {Array<{x: number, y: number}>} data - Um array de objetos com pontos de dados x e y.
 * @returns {{a: number, b: number, c: number, r2: number}} - Um objeto contendo os coeficientes a, b, c e o coeficiente de determinação (r2).
 */
function polynomialRegression(data) {
    ...
    // Para resolver o sistema de equações normais, precisamos das seguintes somas:
    ...
    // Montando a matriz 3x3 (A) e o vetor (B) para o sistema A*sol = B
    const A = [
    ...
    // Resolvendo o sistema de equações lineares para encontrar os coeficientes [a, b, c]
    // Usando uma implementação simples de eliminação gaussiana para matriz 3x3
    for (let i = 0; i < 3; i++) {
        let pivot = i;
        for (let j = i + 1; j < 3; j++) {
    ...
    // --- INÍCIO DA CORREÇÃO ---
    // Se ss_tot é 0, não há variação nos dados do eixo Y, então R² deve ser 0.
...
// --- FIM: FUNÇÃO DE REGRESSÃO POLINOMIAL ---
...
// --- INÍCIO: NOVAS FUNÇÕES DE CONVERSÃO DE COR ---
...
// --- FIM: NOVAS FUNÇÕES DE CONVERSÃO DE COR ---
...
// --- INÍCIO DA ALTERAÇÃO (Etapa A-01) ---

/**
 * Calcula o Erro Padrão da Regressão (Sy/x) para um modelo genérico.
 * @param {Array<{x: number, y: number}>} data - Os dados de calibração originais.
 * @param {Function} predictionFunction - Uma função que aceita um valor 'x' e retorna o 'y' previsto pelo modelo.
 * @param {number} numParameters - O número de parâmetros no modelo (ex: 2 para linear, 3 para polinomial de 2ª ordem).
 * @returns {number|null} O valor de Sy/x, ou null se o cálculo não for possível.
 */
function calculateRegressionStandardError(data, predictionFunction, numParameters) {
    const n = data.length;
    // O número de pontos deve ser maior que o número de parâmetros para calcular os graus de liberdade.
    if (n <= numParameters) {
        log('utils', 'warn', `Não é possível calcular Sy/x: número de pontos (${n}) deve ser maior que o número de parâmetros (${numParameters}).`);
        return null;
    }

    // Calcula a soma dos quadrados dos resíduos (diferença entre o y real e o y previsto).
    const ss_res = data.reduce((sum, point) => {
        const y_predicted = predictionFunction(point.x);
        return sum + Math.pow(point.y - y_predicted, 2);
    }, 0);

    // Graus de liberdade = número de pontos - número de parâmetros do modelo.
...
// --- INÍCIO: NOVAS FUNÇÕES DE VALIDAÇÃO ANALÍTICA (P270624C-01) ---

/**
 * Calcula os erros padrão para um modelo de regressão linear.
 * @param {Array<{x: number, y: number}>} data - Os dados de calibração originais.
 * @param {{m: number, b: number}} modelCoefficients - Os coeficientes do modelo linear.
 * @returns {{Syx: number, Sm: number, Sb: number}} - Um objeto com o erro padrão da regressão (Syx), do coeficiente angular (Sm) e do intercepto (Sb).
 */
function calculateLinearStandardErrors(data, modelCoefficients) {
    ...
    // --- REATORAÇÃO (Etapa A-01): Utiliza a nova função genérica para calcular Syx ---
    const predictionFunction = (x) => m * x + b;
    const Syx = calculateRegressionStandardError(data, predictionFunction, 2); // 2 parâmetros (m, b)

    if (Syx === null) {
        // Se a função genérica falhar (n <= 2), retorna zero para manter o comportamento anterior.
        return { Syx: 0, Sm: 0, Sb: 0 };
    }
    
    // O restante da lógica para Sm e Sb permanece, pois é específica para o modelo linear.
    let sum_x = 0;
    let sum_xx = 0;
    for (const point of data) {
        sum_x += point.x;
...
/**
 * Estima o Limite de Detecção (LOD) e o Limite de Quantificação (LOQ).
 * Usa o método baseado no erro padrão da regressão, uma abordagem comum quando
 * múltiplas leituras do branco não estão disponíveis.
 * @param {number} Syx - O erro padrão da regressão.
 * @param {number} m - O coeficiente angular (sensibilidade) da curva.
 * @returns {{lod: number|null, loq: number|null}} - Um objeto com LOD e LOQ.
 */
function calculateLOD_LOQ(Syx, m) {
    // Adicionada verificação para Syx nulo para maior robustez
    if (m === 0 || Syx === 0 || Syx === null) {
        return { lod: null, loq: null };
    }
    // A sensibilidade (m) deve ser usada em seu valor absoluto para evitar LOD/LOQ negativos.
...
/**
 * Extrai a faixa de concentração utilizada na calibração.
 * @param {Array<{x: number}>} data - Os dados de calibração.
 * @returns {{min: number, max: number}} - O valor mínimo e máximo das concentrações.
 */
...
// --- FIM: NOVAS FUNÇÕES DE VALIDAÇÃO ANALÍTICA ---

// --- INÍCIO DA ALTERAÇÃO: Adição da função Debounce ---
/**
 ...
 * Útil para otimizar eventos repetitivos como redimensionar ou arrastar.
 * @param {Function} func - A função para aplicar o debounce.
 * @param {number} delay - O tempo de espera em milissegundos antes da execução.
 * @returns {Function} A nova função "debounced".
 */
function debounce(func, delay) {
    let timeoutId;

    // Retorna uma nova função que encapsula a original
    return function(...args) {
        // 'this' e 'args' são preservados para a chamada original
        const context = this;

        // Limpa o timeout anterior sempre que a função é chamada
        clearTimeout(timeoutId);

        // Define um novo timeout
        timeoutId = setTimeout(() => {
            // Executa a função original com o contexto e argumentos corretos
