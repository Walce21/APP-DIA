...
function calculateAllMetrics(allRois, whiteRoi, imageData) {
    ...
    allRois.forEach(roi => {
        // --- INÍCIO DA CORREÇÃO ---
        // A verificação foi alterada de '!roi.rgb' para '!roi.pixelCount || roi.pixelCount === 0'.
        // Isso garante que ROIs que foram criadas mas ainda não foram processadas (assincronamente)
        // tenham seus dados de cor calculados aqui, antes de prosseguir.
        if (!roi.pixelCount || roi.pixelCount === 0) {
        // --- FIM DA CORREÇÃO ---
